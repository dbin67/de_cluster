...
run_hadoop:
	...
	docker run -d -p 9870:9870 -h namenode --name namenode --network ${DOCKER_NETWORK} hadoop /scripts/namenode.sh
	docker run -d -p 8088:8088 -h resourcemanager --name resourcemanager --network ${DOCKER_NETWORK} hadoop /scripts/resourcemanager.sh
...
run_hbase:
	...
	docker run -d -p 16010:16010 -h hmaster --name hmaster --network ${DOCKER_NETWORK} hbase /scripts/hmaster.sh
	docker run -d -p 9090:9090 -h thrift --name thrift --network ${DOCKER_NETWORK} hbase /scripts/thrift.sh
